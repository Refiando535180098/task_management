...
export default function App() {
  ...
  // --- REALTIME SUBSCRIPTION (MENGGANTIKAN REFRESH OTOMATIS) ---
  useEffect(() => {
    if (currentUser) {
      loadTasksFromDB();
      fetchNotifications();
  ...
  // --- LOGIKA HAPUS NOTIFIKASI SAAT DIBACA ---
      ...
      if (!error && newTasks && newTasks.length > 0) {
        ...
        const notifsToInsert = assignedUserIds
           ...
           .map(id => ({
             userId: id,
             type: 'task',
             message: `Tugas Baru: "${newTasks[0].title}" (${newTasks[0].priority.toUpperCase()})`,
             read_status: false,
